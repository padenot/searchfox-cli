...
def main():
    ...
    results = client.search(query="AudioNode", path="^dom/media", cpp=True, limit=3)
    for path, line_num, line in results:
        print(f"{path}:{line_num}: {line}")
    ...
    try:
        call_graph = client.search_call_graph(
            calls_from="mozilla::dom::AudioContext::CreateGain", depth=2
        )
        if call_graph:
    ...
    results = client.search(query="^Create", regexp=True, path="dom/media", limit=5)
    for path, line_num, line in results:
        print(f"{path}:{line_num}: {line}")

    # Example 10: Performance monitoring
    separator("Performance: Measure search latency")
    client_with_logging = searchfox.SearchfoxClient(
        "mozilla-central", log_requests=True
    )
...
def advanced_example():
    ...
    results = client.search(
        query="class .* : public AudioNode", regexp=True, cpp=True, limit=10
    )
        ...
        if "class " in line and ": public AudioNode" in line:
            ...
            if "class" in parts:
                ...
                if idx + 1 < len(parts):
                    ...
                    audio_nodes.append(
                        {"name": class_name, "path": path, "line": line_num}
                    )
